...
float[] DivY = new float[numberOfButtons-2];
float[] DivWidth = new float[numberOfButtons-2];
float[] DivHeight = new float[numberOfButtons-2];
//
for ( int i=0; i<DivX.length; i++ ) {
  DivY[i] = buttonY;
  DivWidth[i] = widthOfButton;
  DivHeight[i] = widthOfButton;
}
for ( int i=0; i<DivX.length; i++ ) {
...
//
//STOP BUTTON, #0
float stopButtonX = DivX[0] + DivWidth[0]*1/4;
float stopButtonY = DivY[0] + DivHeight[0]*1/4;
...
float muteTopPoint1X = DivX[1] + DivWidth[1]*1/4 ;
float muteTopPoint1Y = DivY[1] + DivHeight[1]*1/4 ;
float muteTopPoint2X = muteTopPoint1X + DivWidth[1]*1/2 ; //based on beginning of line
float muteTopPoint2Y = muteTopPoint1Y ;
float muteDownPoint1X = DivX[1] + DivWidth[1]*3/4 ;
float muteDownPoint1Y = DivY[1] + DivHeight[1]*1/4 ;
float muteDownPoint2X = muteDownPoint1X; //based on beginning of line
float muteDownPoint2Y = DivY[1] + DivHeight[1]*3/4 ;
float muteBottomPoint1X = DivX[1] + DivWidth[1]*3/4 ;
float muteBottomPoint1Y = DivY[1] + DivHeight[1]*3/4;
float muteBottomPoint2X = DivX[1] + DivWidth[1]*1/4 ; //based on beginning of line
float muteBottomPoint2Y = DivY[1] + DivHeight[1]*3/4 ;
float muteUpPoint1X = DivX[1] + DivWidth[1]*1/4;
float muteUpPoint1Y = DivY[1] + DivHeight[1]*3/4;
float muteUpPoint2X = muteUpPoint1X ; //based on beginning of line
float muteUpPoint2Y = DivY[1] + DivHeight[1]*1/4;
...
//
//Previous Button, #2
float prevX1 = DivX[2] + DivWidth[2]*1/4 + DivWidth[2]*1/2;
float prevY1 = DivY[2] + DivHeight[2]*1/4;
float prevX2 = prevX1 - DivWidth[2]*1/4;
float prevY2 = prevY1 + DivHeight[2]*1/4 ;
float prevX3 = prevX1 ;
float prevY3 = prevY2 + DivHeight[2]*1/4;
float prevX = DivX[2] + DivWidth[2]*1/2 - DivWidth[2]*1/8 ;
float prevY = DivY[2] + DivHeight[2]*1/4 ;
...
float fastRewindX2 = fastRewindX1 - DivWidth[3]*1/4 ;
float fastRewindY2 = fastRewindY1 + DivHeight[3]*1/4 ;
float fastRewindX3 = fastRewindX1 ;
float fastRewindY3 = fastRewindY2 + DivHeight[3]*1/4;
float fastRewindX4 = fastRewindX2 ;
float fastRewindY4 = fastRewindY1 ;
float fastRewindX5 = fastRewindX2 - DivWidth[3]*1/4 ;
float fastRewindY5 = fastRewindY2 ;
float fastRewindX6 = fastRewindX2 ;
float fastRewindY6 = fastRewindY3 ;
//
//Pause Button, #4
float pauseX1 = DivX[4] + DivWidth[4]*1/4;
float pauseY1 = DivY[4] + DivHeight[4]*1/4;
float pauseX2 = DivX[4] + DivWidth[4] - DivWidth[4]*1/4 - DivWidth[4]*1/8; //Right Point Start
float pauseY2 = pauseY1;
float pauseWidth1 = DivWidth[4]*1/8;
float pauseWidth2 = DivWidth[4]*1/8;
...
float playX2 = playX1 + DivWidth[5]*1/2;
float playY2 = playY1 + DivHeight[5]*1/4;
float playX3 = playX1;
float playY3 = playY2 + DivHeight[5]*1/4;
//
//loop once button, #6
//Note, this is a stop botton with a "one inside" & a triangle
//CAUTION: this needs text
float loopOnceX = DivX[6] + DivWidth[6]*1/4 ;
float loopOnceY = DivY[6] + DivHeight[6]*1/4 ;
float loopOnceWidth = DivWidth[6]*1/2;
float loopOnceHeight = DivHeight[6]*1/2;
float cornerX = loopOnceX + loopOnceWidth;
float cornerY = loopOnceY;
float loopOnceX1 = cornerX - DivWidth[6]*1/16 ;
float loopOnceY1 = cornerY - DivHeight[6]*1/16 ;
float loopOnceX2 = cornerX + DivWidth[6]*1/16 ;
float loopOnceY2 = cornerY ;
float loopOnceX3 = loopOnceX1 ;
float loopOnceY3 = cornerY + DivHeight[6]*1/16 ;
//
//loop infinite button, #7
//Note: Loop infinite button is same as loop once, without text "1"
float loopInfiniteX = DivX[7] + DivWidth[7]*1/4 ;
float loopInfiniteY = DivY[7] + DivHeight[7]*1/4 ;
float loopInfiniteWidth = DivWidth[7]*1/2;
float loopInfiniteHeight = DivHeight[7]*1/2;
cornerX = loopInfiniteX + loopInfiniteWidth ;
cornerY = loopInfiniteY ;
float loopInfiniteX1 = cornerX - DivWidth[7]*1/16 ;
float loopInfiniteY1 = cornerY - DivHeight[7]*1/16 ;
float loopInfiniteX2 = cornerX + DivWidth[7]*1/16 ;
...
float fastForwardX2 = fastForwardX1 + DivWidth[8]*1/4;
float fastForwardY2 = fastForwardY1 + DivHeight[8]*1/4;
float fastForwardX3 = fastForwardX1 ;
float fastForwardY3 = fastForwardY2 + DivHeight[8]*1/4;
float fastForwardX4 = fastForwardX2;
float fastForwardY4 = fastForwardY1;
float fastForwardX5 = fastForwardX4 + DivWidth[8]*1/4;
float fastForwardY5 = fastForwardY2;
float fastForwardX6 = fastForwardX4;
float fastForwardY6 = fastForwardY3;
//
//Next Button, #9
float nextX1 = DivX[9] + DivWidth[9]*1/4 + 0*DivWidth[9]*1/2;
float nextY1 = DivY[9] + DivHeight[9]*1/4;
float nextX2 = nextX1 + DivWidth[9]*1/4;
float nextY2 = nextY1 + DivHeight[9]*1/4;
float nextX3 = nextX1 ;
float nextY3 = nextY2 + DivHeight[9]*1/4;
float nextX = nextX2;
float nextY = nextY1;
float nextWidth = DivWidth[9]*1/8;
float nextHeight = DivHeight[9]*1/2;
//
//Shuffle Button, #10
float shuffleCross1X1 = DivX[10] + DivWidth[10]*1/4;
float shuffleCross1Y1 = DivY[10] + DivHeight[10]*1/4;
float shuffleCross1X2 = DivX[10] + DivWidth[10]*3/4;
float shuffleCross1Y2 = DivY[10] + DivHeight[10]*3/4;
float shuffleCross2X1 = DivX[10] + DivWidth[10]*1/4;
float shuffleCross2Y1 = DivY[10] + DivHeight[10]*3/4;
float shuffleCross2X2 = DivX[10] + DivWidth[10]*3/4;
float shuffleCross2Y2 = DivY[10] + DivHeight[10]*1/4;
float cornerX1 = shuffleCross2X2 ;
float cornerY1 = shuffleCross2Y2 ;
float shuffleTriX1 = cornerX1;
float shuffleTriY1 = cornerY1 - DivHeight[10]*1/16;
float shuffleTriX2 = cornerX1;
float shuffleTriY2 = cornerY1 + DivHeight[10]*1/16;
float shuffleTriX3 = cornerX1 + DivWidth[10]*1/16;
float shuffleTriY3 = cornerY1;
float cornerX2 = shuffleCross1X2 ;
float cornerY2 = shuffleCross1Y2 ;
float shuffleTriX4 = cornerX2;
float shuffleTriY4 = cornerY2 - DivHeight[10]*1/16;
float shuffleTriX5 = cornerX2;
float shuffleTriY5 = cornerY2 + DivHeight[10]*1/16;
float shuffleTriX6 = cornerX2 + DivWidth[10]*1/16;
float shuffleTriY6 = cornerY2;
//
float musicSongPaddingY = widthOfButton*1/4;
float musicSongSpaceX = DivX[0];
float musicSongSpaceY = DivY[0] + widthOfButton + musicSongPaddingY;
...
rect(DivX[0], DivY[0], DivWidth[0], DivHeight[0]);  //*0
rect(DivX[1], DivY[1], DivWidth[1], DivHeight[1]); //*1
rect(DivX[2], DivY[2], DivWidth[2], DivHeight[2]); //*2
rect(DivX[3], DivY[3], DivWidth[3], DivHeight[3]); //*3
rect(DivX[4], DivY[4], DivWidth[4], DivHeight[4]); //*4
rect(DivX[5], DivY[5], DivWidth[5], DivHeight[5]); //*5
rect(DivX[6], DivY[6], DivWidth[6], DivHeight[6]);
rect(DivX[7], DivY[7], DivWidth[7], DivHeight[7]);
rect(DivX[8], DivY[8], DivWidth[8], DivHeight[8]);
rect(DivX[9], DivY[9], DivWidth[9], DivHeight[9]);
rect(DivX[10], DivY[10], DivWidth[10], DivHeight[10]);
