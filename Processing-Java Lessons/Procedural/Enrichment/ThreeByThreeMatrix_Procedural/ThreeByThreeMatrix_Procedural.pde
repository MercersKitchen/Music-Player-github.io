  ...
  for ( int x=0; x<columns; x++ ) {//Will fail if not a mutliple
    for ( int y=0; y<rows; y++ ) { //Will fail if not a multiple
      if ( rectVar.length%parameters==0 ) {
        for ( int i=0; i<rectVar.length; i++ ) { //Will fail if not a multiple
        if (i%4==0) populateArray(rectVar, i, x, appWidth, columns);
        if (i%4==1) populateArray(rectVar, i, y, appHeight, rows);
        if (i%4==2) populateArray(rectVar, i, 1, appWidth, columns);
        if (i%4==3) populateArray(rectVar, i, 1, appHeight, rows);
